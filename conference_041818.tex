...
%引出formal semantics
 ...
 %In this paper, we are focusing on achieving the goal of formally analyzing systems implemented in mixed language.
...
%$<(\textbf{BEQ}~X,~Mem,~GReg,~SReg,~Stack,~fun>~\rightarrow\\ \\$

%TODO:Semantics for BEQ and CMP
...
\end{align}
%\par $\mathbb{K}$ is a rewriting-based programming language semantic framework for defining programming languages, type systems and formal analysis tools using configurations and rules. In $\mathbb{K}$, the definition of a programming language
%consists of three parts: the syntax, configuration and the semantics. The syntax
%is given in the form of conventional Backus-Naur Form (BNF). A configuration
%is a set of labelled, potentially nested units (called cells). It
%represents the state of the running program and its executing
%context such as memory, environment, etc. The semantics is defined using rewrite rules (also
%called semantic rules) over configurations. A rewrite rule describes
%the one-step transition from a configuration to another
%configuration. Formally, a rewrite rule is of the form C $\rightarrow$ $C^`$, where C and $C^`$ are configurations. If the configuration on the left-hand side (e.g., C) of a rewrite rule matches the current configuration, the rule fires and rewrites the current configuration as specified by its right-hand side (e.g., $C^`$). Multiple
%rewrite rules can fire at the same time. For better understanding
%of rewrite rules, consider the following example:
...
%But if there will be something to be added in this cell, it should have type of Map.
...
%In the rule, there are two cells  \term{k} and \term{env}. The \term{k} cell represents a list (or stack) of computations waiting to be performed. The left-most (i.e., top) element of the stack is the next item to be computed. The \term{env} cell is a map of variables to their locations.
