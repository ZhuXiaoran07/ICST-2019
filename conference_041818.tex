...
\par The instruction $BEQ$ often follows some logical instructions, such as $CMP$. In case the \\\\
...
To reduce the verification efforts, automatic OS kernel verification has long been a research objective. \cite{hyperkernel} translates both C and assembler implementations to LLVM \cite{llvm} and adopts the LLVM verifier to verify the correctness of the OS kernel. \cite{jizheng} abstract the assembler programs to C level and verify the OS kernel with the C verifier. \cite{Ironclad} compiles C to assembler and conducts all
